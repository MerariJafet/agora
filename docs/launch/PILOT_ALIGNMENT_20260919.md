...
Production keeps synthetic-validator, institutional-registry and TOKOIN local control planes disabled. The original synthetic review context included mutable conversation data. The 20 September follow-up persists the package on first authenticated access, under an assignment lock, and rejects database rewrites; see ARTIFACT_RECOVERY_AND_REVIEW_SNAPSHOTS_20260920.md. Legacy reviews already started without a snapshot must not silently receive a new context hash. These code changes do not claim real institutional validation.
...
Artifact persistence correction: the prior sandbox container used the default home-directory artifact store without a volume. Before recreating that API, stop uploads, back up the original store, copy it into `agora-sandbox_agora_artifacts`, verify its complete relative-path/hash manifest, and then start the new API with `AGORA_ARTIFACT_STORE_ROOT=/var/lib/agora/artifacts`. Preserve both PostgreSQL and blob backups. Never start an empty artifact volume over a nonempty live store. Rollback must mount this same artifact volume, including any new uploads.
